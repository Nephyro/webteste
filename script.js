window.addEventListener('load', () => {
    ...
    const statusUpdates = [
        "> INICIANDO_NÓ...", 
        "> CARREGANDO_REGISTROS...", 
        "> AUTENTICANDO...", 
        "> MAPEANDO_SISTEMA...", 
        "> ACESSO_PERMITIDO."
    ];

    // --- 1. MOTOR DO TESSERACT ASCII ---
    let angle = 0;
    ...
    for (let i = 0; i < 16; i++) {
        for (let j = i + 1; j < 16; j++) {
            if ([vertices4D[i].x!==vertices4D[j].x, vertices4D[i].y!==vertices4D[j].y, 
                 vertices4D[i].z!==vertices4D[j].z, vertices4D[i].w!==vertices4D[j].w].filter(Boolean).length === 1) 
                edges.push([i, j]);
        }
    }

    function renderTesseract() {
        if (!asciiTesseract) return;
        buffer.fill(' ');
        const projected = vertices4D.map(v => {
            const cosT = Math.cos(angle), sinT = Math.sin(angle);
            let x = v.x * cosT - v.y * sinT, y = v.x * sinT + v.y * cosT, z = v.z, w = v.w;
            const zr = z * Math.cos(angle * 0.5) - w * Math.sin(angle * 0.5);
            const wr = z * Math.sin(angle * 0.5) + w * Math.cos(angle * 0.5);
            const f4 = 1 / (3 + wr), f3 = 1 / (4 + zr * f4);
            return { 
                x: Math.floor(width/2 + 60 * f3 * x * f4 * 2.2), 
                y: Math.floor(height/2 + 60 * f3 * y * f4) 
            };
        });
        edges.forEach(edge => {
            let x0 = projected[edge[0]].x, y0 = projected[edge[0]].y;
            let x1 = projected[edge[1]].x, y1 = projected[edge[1]].y;
    ...
    // --- 3. LÓGICA DO TERMINAL (CORRIGIDA) ---
    const terminalData = [
        { text: '>>> ', color: '#7289a3' }, 
        { text: 'print', color: '#ff1f57' }, 
        { text: '(', color: '#fff' }, 
        { text: '"Hello World"', color: '#f1fa8c' }, 
        { text: ')', color: '#fff' }, 
        { text: '\nHello World', color: '#fff' }, 
        { text: '\n>>> ', color: '#7289a3' }
    ];
        ...
        function step() {
            if (pIdx < terminalData.length) {
                if (cIdx === 0) {
                    ...
                    s.style.color = terminalData[pIdx].color;
                    s.id = `term-${pIdx}`;
                    terminalEl.appendChild(s);
                }
                const currentSpan = document.getElementById(`term-${pIdx}`);
                currentSpan.textContent += terminalData[pIdx].text.charAt(cIdx++);
    ...
    // --- 4. EFEITOS DE MOUSE (TILT & CURSOR) ---
    ...
    // --- 5. OBSERVERS DE ROLAGEM ---
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => { 
            if(entry.isIntersecting) { 
                if(entry.target.id === 'terminal-code') {
                    typeTerminal();
                    observer.unobserve(entry.target); // Resolve o erro de duplicação
                } else {
